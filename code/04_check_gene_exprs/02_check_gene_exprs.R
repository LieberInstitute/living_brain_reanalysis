...
colnames(colData(rse_gene)) = gsub("%","perc", colnames(colData(rse_gene)),fixed=TRUE)
...
write.csv(as.data.frame(colData(rse_gene)), file = "phenotype/LBP_merged_seqmetrics.csv",
	row.names=FALSE)
...
## extract
outGene = topTable(eBGene, coef =2, p.value = 1, sort = "none", number=nrow(dge))
...
outGene_qsva = topTable(eBGene_qsva, coef =2, p.value = 1, sort = "none", number=nrow(dge))
...
outGene_pc3 = topTable(eBGene_pc3, coef =2, p.value = 1, sort = "none", number=nrow(dge))
...
par(mar=c(5,6,4,2), cex.axis=2,cex.lab=2)
res_match = res[match(outGene$ensembl_id, res$ensembl_id),]
plot(res_match$t, outGene$t, xlab = "Pre-print DE t-stat",
...
par(mar=c(5,6,4,2), cex.axis=2,cex.lab=2)
plot(outGene$t[outGene$isExprs] ~ outGene_qsva$t[outGene_qsva$isExprs],
	pch = 21, bg="grey", xlab = "PM vs LIV t-stat (with qSVA)",
	ylab= "PM vs LIV t-stat (without qSVA)")
abline(0,1,col="blue", lwd=3, lty=2)

plot(outGene$logFC[outGene$isExprs] ~ outGene_qsva$logFC[outGene_qsva$isExprs],
	pch = 21, bg="grey", xlab = "PM vs LIV log2FC (with qSVA)",
...
## model
mod_liv = model.matrix(~ samplingage + race + sex + diagnosis +
...
## extract
outGene_liv = topTable(eBGene_liv, coef =2, p.value = 1, sort = "none", number=nrow(dge))
...
outGene_qsva_liv = topTable(eBGene_liv_qsva, coef =2, p.value = 1, sort = "none", number=nrow(dge))
outGene_qsva_liv$isExprs = outGene_qsva_liv$ensembl_id %in% res$ensembl_id
outGene_qsva_liv$adj.P.Val = NA
outGene_qsva_liv$adj.P.Val[outGene_qsva_liv$isExprs] = p.adjust(
						outGene_qsva_liv$P.Value[outGene_qsva_liv$isExprs] , "fdr")
...
## model
mod_pm = model.matrix(~ samplingage + race + sex + diagnosis +
		rin + recount_qc.star.uniquely_mapped_reads_perc + cellPC +
		recount_qc.aligned_readsperc.chrm +
		recount_qc.aligned_readsperc.chrx +
		recount_qc.bc_auc.all_perc +
...
## extract
outGene_pm = topTable(eBGene_pm, coef =2, p.value = 1, sort = "none", number=nrow(dge))
...
outGene_qsva_pm = topTable(eBGene_pm_qsva, coef =2, p.value = 1, sort = "none", number=nrow(dge))
outGene_qsva_pm$isExprs = outGene_qsva_pm$ensembl_id %in% res$ensembl_id
outGene_qsva_pm$adj.P.Val = NA
outGene_qsva_pm$adj.P.Val[outGene_qsva_pm$isExprs] = p.adjust(
						outGene_qsva_pm$P.Value[outGene_qsva_pm$isExprs] , "fdr")

table(outGene_pm$adj.P.Val < 0.05)
table(outGene_qsva_pm$adj.P.Val < 0.05)


plot(outGene_pm$logFC[outGene_pm$isExprs], outGene_liv$logFC[outGene_pm$isExprs],
	xlab = "Age logFC (PM, no qSVA)", ylab = "Age logFC (LIV, no qSVA)",
	pch = 21, bg = "grey")
cor.test(outGene_pm$logFC[outGene_pm$isExprs], outGene_liv$logFC[outGene_pm$isExprs])
plot(outGene_qsva_pm$logFC[outGene_pm$isExprs], outGene_qsva_liv$logFC[outGene_pm$isExprs],
	xlab = "Age logFC (PM, qSVA)", ylab = "Age logFC (LIV, qSVA)",
	pch = 21, bg = "grey")
