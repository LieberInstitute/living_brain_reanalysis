...
pcHeatmap = function (rse, pca, plotdir, pthresh = 10, width = 12, height = 14) {
    ...
    colnames(pvalPcaMetrics) = gsub("recount_qc.", "", colnames(pvalPcaMetrics),fixed = TRUE)
    ...
    print(levelplot(logPvalMat, aspect = "fill", at = theSeq,
        pretty = TRUE, xlab = "", ylab = "", scales = list(x = list(rot = 90,
            cex = 1.2), y = list(cex = 1.2)), panel = panel.levelplot.raster,
        col.regions = my.col))
    dev.off()
}
...
###### example plots
vars = colnames(colData(rse_gene))[sapply(colData(rse_gene), class) == "numeric"]
vars = vars[!grepl("postmortem", vars)]
ccPcaMetrics = cor(pca$x[, 1:10], as.data.frame(colData(rse_gene)[,vars]))
...
par(mar=c(4,6,2,2), cex.axis=2,cex.lab=2)
palette(brewer.pal(4,"Dark2"))
vars_nam = gsub("recount_qc.","", vars, fixed=TRUE)
for(i in seq(along=vars)) {
	boxplot(colData(rse_gene)[,vars[i]] ~ rse_gene$COI,
		ylab = vars_nam[i], xlab = "", outline=FALSE,
		ylim = range(colData(rse_gene)[,vars[i]] ))
	points(colData(rse_gene)[,vars[i]]  ~
		jitter(as.numeric(rse_gene$COI), amount=0.15),
		pch = 21, bg = rse_gene$COI)
	tt = summary(lm(colData(rse_gene)[,vars[i]] ~ rse_gene$COI))$coef[2,]
...
par(mar=c(4,6,2,2), cex.axis=1.7,cex.lab=1.7)
palette(brewer.pal(4,"Dark2"))
boxplot(pca$x[,1] ~ rse_gene$COI, xlab = "",
	ylab = paste0("PC1: ", pcaVars[1], "% Var Expl"),
	ylim = range(pca$x[,1]), outline=FALSE)
...
par(mar=c(4,6,2,2), cex.axis=1.7,cex.lab=1.7)
palette(brewer.pal(4,"Dark2"))
plot(pca$x[,1] ~ colData(rse_gene)[,nn[1]],
...
par(mar=c(4,6,2,2), cex.axis=1.7,cex.lab=1.7)
palette(brewer.pal(4,"Dark2"))
boxplot(colData(rse_gene)[,nn[1]] ~ rse_gene$COI,
	ylab = gsub("recount_qc.", "", nn[1], fixed=TRUE),
	xlab = "", outline=FALSE,
	ylim = range(colData(rse_gene)[,nn[1]] ))
points(colData(rse_gene)[,nn[1]]  ~
	jitter(as.numeric(rse_gene$COI), amount=0.15),
	pch = 21, bg = rse_gene$COI)
dev.off()
...
par(mar=c(4,6,2,2), cex.axis=1.7,cex.lab=1.7)
palette(brewer.pal(4,"Dark2"))
plot(pca$x[,2] ~ colData(rse_gene)[,nn[2]],
...
par(mar=c(4,6,2,2), cex.axis=1.7,cex.lab=1.7)
palette(brewer.pal(4,"Dark2"))
boxplot(pca$x[,3] ~ rse_gene$COI, xlab = "",
	ylab = paste0("PC3: ", pcaVars[3], "% Var Expl"),
	ylim = range(pca$x[,3]), outline=FALSE)
...
par(mar=c(4,6,2,2), cex.axis=1.7,cex.lab=1.7)
palette(brewer.pal(4,"Dark2"))
plot(pca$x[,3] ~ colData(rse_gene)[,nn[2]],
	xlab = gsub("recount_qc.", "", nn[2], fixed=TRUE),
	ylab = paste0("PC3: ", pcaVars[3], "% Var Expl"),
	pch = 21, bg = rse_gene$COI)
for(i in seq(along=gIndexes)) {
...
par(mar=c(4,6,2,2), cex.axis=1.7,cex.lab=1.7)
palette(brewer.pal(4,"Dark2"))
boxplot(colData(rse_gene)[,nn[2]] ~ rse_gene$COI,
	ylab = gsub("recount_qc.", "", nn[2], fixed=TRUE),
	xlab = "", outline=FALSE,
	ylim = range(colData(rse_gene)[,nn[2]] ))
points(colData(rse_gene)[,nn[2]]  ~
	jitter(as.numeric(rse_gene$COI), amount=0.15),
	pch = 21, bg = rse_gene$COI)
dev.off()
